...
WebUI 的 HTML / CSS / JavaScript **禁止嵌入 Python 字符串**，必须独立文件：

- HTML 模板 → `paimon/channels/webui/templates/<page>.html`
- CSS → `paimon/channels/webui/static/css/<page>.css`
- JS → `paimon/channels/webui/static/js/<page>.js`
- 图片 / 字体 → `paimon/channels/webui/static/{img,fonts}/`

Python 文件只负责：路由 handler、数据组装、模板渲染上下文（调 `render(name, ctx)`）。

违例信号 — 见到立即拆出去：
- `_X_css_*.py` / `_X_script_*.py` / `_X_body.py` 这种 page 子包内的字符串文件
- `*_html.py` 顶层文件超 200 行（说明 HTML + CSS + JS 还塞在 Python 里）
- 字符串里大段 `<div>` / `function ()` / CSS 属性

新增 page 套路：
1. `templates/<page>.html` 写 HTML（用 `{{var}}` / `{{var|safe}}` / `{{include:_nav}}` 占位）
2. `static/css/<page>.css` 写 page 特定样式（通用样式直接用 components.css 的 `.pm-*` class）
3. `static/js/<page>.js` 写 page 特定脚本（异步按钮统一调 `pmBtn.runAsync(btn, fn, opts)`）
4. `paimon/channels/webui/api/<page>.py` 加 handler，调 `render('<page>', ctx)` + `_layout` 返回

通用控件清单（在 `components.css` + `components.js`）：
Button / Input / Textarea / Select / Card / Stat-Card / Tab / Modal / Toast / Spinner / Badge / Empty State / Table / Nav / Form Group。**所有页面禁止重复造**。
