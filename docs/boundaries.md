...
| 场景 | 归属 | 关键区分点 |
|---|---|---|
| 采集结果整理 | 数据收集者（风神 / 岩神 等）| 谁的数据谁整理；落各自业务表（dividend_snapshot / daily_hotspot 等）|
| 事件历史归档 | 各 archon 直接调 `irminsul.push_archive_create` | 落 push_archive 表，面板按 actor 拉历史展示，**不主动通知** |
| 用户对话推送 | 派蒙 | 主动通知一律走对话回话（如 /agents 纪要 / 自进化提案）|
