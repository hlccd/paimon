...
- 看死执评语 + 草案全文
- 同意 → 冰神 apply 落 `<repo>/skills/<name>/SKILL.md` + 注册 → 立即可用
- 拒绝 → status=rejected（30 天后 cron 清）

archive hook 自动触发版无 webui 提示，提案直接落 `/plugins` 面板待审。月度 cron 同。
...
| 历史任务查询 | `/tasks` 面板 + `/task-list` | `/task-list` + `/task-index` |
...
历史任务（含自进化触发的内部 task）查询走 `/task-list` + `/task-index`。

### 摘要 fallback 链（QQ / WebUI 共用）

`/task-index` 走 [`paimon/shades/_task_summary.py`](../paimon/shades/_task_summary.py)，按信息密度逐级回退：
...
- **"自进化提案产出怎么知道？"**——派蒙在 /evolve 命令完成后回一条文本指向 `/plugins#proposals`；archive hook 自动触发的提案直接落面板，无对话流提示。
