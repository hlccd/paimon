...
> 隶属：[神圣规划](aimon.md)
>
> ⚠️ **已归档（2026-04-29 迁移完成）**：本文档是历史执行手册，迁移工作已全部完成（参见 [todo.md](todo.md) §3 与 [progress.md](progress.md)）。
> 文中 `fairy/*` 路径是迁移前的代号——实际项目代号已为 `paimon/`。下文 P0/P1 待办、Phase 验收条件等都已落地，保留作为历史方案参考；不要按文中"待执行"的步骤再操作。
> 当前实现状态详见 [progress.md](progress.md)；剩余待办见 [todo.md](todo.md)。
>
