...
- 影响：用户在 skill 调用 30s+ 工具循环里发 /stop 无法打断（只能等超时 / 流错）
- 修复路径：`/stop` 扩展为按 channel_key 反查所有活跃任务（主 session + 当前 channel 上的 ephemeral session）批量 cancel；或 ephemeral session 创建时也注册到 channel_key 副表
- 优先级：低（skill 一般 ≤30s，超时也会自然结束）
- 文件：`paimon/core/chat/session.py:stop_session_task`、`paimon/core/commands/_dispatch.py:_run_skill_isolated`
