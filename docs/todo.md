...
### 6. skill 调用应隔离 session 历史，不污染主对话

- 现状：`/<skill_name>` 兜底 `_invoke_skill` 复用主 session（`session_mgr.get_current`）→ LLM 看到完整 chat history
- 问题：连续两次 `/topic <X>` `/topic <Y>` 时 LLM 会基于历史做 X vs Y 跨轮对比；重复触发 skill 的污染会越积越多
- 当前 workaround：在 SKILL.md 加「独立调研原则」明确禁止跨轮对比（A 方案）
- 真正修复：skill 调用建临时 isolated session（B 方案）—— 不读主 session messages / 不写主 session / 跑完丢弃临时 session
- 副作用考量：失去"基于上一轮 skill 结果继续追问"的能力——但 user 主流诉求是"每次独立"
- 范围：所有 skill 都受益，不止 topic
- 文件：`paimon/core/commands/_dispatch.py:_invoke_skill`
