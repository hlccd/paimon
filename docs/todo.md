...
- [ ] **权限体系 v2 重新设计**（2026-04-28 起草）—— 当前授权流程对单用户自用场景过度防御：每次重启都要给 builtin skill / 7 个 archon 重新授权，体验差。临时缓解：启动时把"已加载的 builtin skill + 7 个 archon"自动写 permanent_allow（`permanent_deny` 的不覆盖）；运行时新增的 plugin / AI 生成 skill 仍走死执 review。**根本设计待重做**：思路方向 (1) 工具粒度：skill 不问，tool 调用按参数模式问（`Bash(rm:*)`、`Bash(curl http*download*)` 这类）(2) 取消 archon 神名作 subject（粒度太粗）(3) 对单用户场景考虑分级：「自用模式」全自动放行 + 真破坏命令依赖 pre_filter / 「严格模式」按现状询问。先用临时缓解度过痛点，正式重构等需求更明确。
...
- [x] ~~**岩神增强·公告区 + 日报 v1**~~ —— 2026-04-28：规则驱动事件分级（`_classify_event_severity`/`_aggregate_events` 按 P0/P1/P2 分档）+ markdown 日报生成器 `_compose_daily_digest` + push_archive(source="岩神·理财日报", extra={p0/p1_count})；`/api/wealth/running` 暴露采集状态；wealth_html 公告区对齐 sentiment（日期切换 ← → 今天、采集中状态条、按日展开、2s 自轮询）+ stat 卡改"近 7 天 P0+P1"
- [ ] **岩神增强·第二期（事件化）**（2026-04-28 起草）—— 第一期用"规则 + push_archive.extra"快速出了公告区，但事件本身未落独立表。第二期把事件化数据模型接起来：
  - (a) 世界树新增 `dividend_events` 域（对标 `feed_events`）：字段 severity/event_type/title/summary/timeline/entities/first_seen_at/last_seen_at/status/sentiment_score
  - (b) `_aggregate_events` 结果 upsert 到新域，同股同 type 7 天内 merge 一条（而不是每次 scan 都新建）
  - (c) WebUI 新增"理财事件"tab：按 severity 分层展示（P0 顶 + 红 border），点进 modal 看完整 timeline
  - (d) `_compose_daily_digest` 从 `dividend_events` 表取代替从 scan_result 现算（解耦日报与单次采集）
  - (e) 历史事件查询 API：`/api/wealth/events?severity=p0&days=30` 支持按维度筛选
  - **预估**：世界树加表 ~200 行 + 岩神事件化 ~150 行 + UI tab ~200 行。完成后岩神体验对标风神 sentiment 面板
