...
| 任务 | 来源 | 说明 | 状态 |
|---|---|---|---|
| 定时调度核心 | **参考重写** fairy/scheduler/ | cron/interval/once 三种触发；按分钟 :00 对齐轮询，MIN_INTERVAL=60s | ✅ 2026-04-23 |
| 推送响铃（定时） | **新建** | 三月 cron 到点 → 地脉 march.ring → 派蒙投递；WebUI 侧 `send_text` + PushHub 扇出 SSE | ✅ 2026-04-23 |
| 推送响铃（事件） | **新建** | 数据收集者请求三月响铃 → 派蒙送达 | ⏳ 未做（依赖风神/岩神 Phase 4 扩展） |
| 推送积压 | **新建** | 派蒙挂掉时暂存；恢复后补发 | 部分 — 推送会话消息持久化到世界树（离线可补读）；派蒙挂起积压队列未实装 |
| 自检系统 | **参考重写** fairy/diagnostics.py | 理解诊断逻辑后重新实现 | ⏳ 未做 |
| 守护进程 | **新建** | 监控派蒙存活；异常拉起 | ✅ 早期实装 |
| 任务观测面板数据 | **新建** | 收集运行状态 → 供 WebUI 观测面板展示 | ✅ 早期实装 |
| **单元测试** | 新建 | 调度核心测试、响铃机制测试、积压队列测试、自检系统测试、守护进程测试 | ⏳ 未做 |
...
| 任务 | 说明 | 状态 |
|---|---|---|
| 永久授权存储 | 世界树中的授权条目 schema 落地 | ✅ 2026-04-23 `authz_records` 表，三元组 `(subject_type, subject_id, user_id)` |
| 天使路径权限 | 派蒙单项询问 + 永久关键词识别 + 写世界树 | ✅ 2026-04-23 `paimon/core/authz/` 四件套 + `Channel.ask_user` |
| 四影路径权限 | 死执 DAG 批量扫描 + 一次性打包询问 | ⏳ 未做（依赖 Phase 3B 生执 DAG 完善） |
| 授权缓存同步 | 面板撤销 → 地脉事件 → 派蒙/死执缓存失效 | ✅ 2026-04-23 冰神面板撤销 → `authz_cache.invalidate()` 同模块闭环 + 订阅 `skill.loaded` |

**额外（非原计划但已实装）**：**敏感度自动派生** —— 冰神装载时按 skill 的 `allowed_tools` 对照 `SENSITIVE_TOOLS` 清单自动派生 sensitivity，无需手填 manifest。

#### 6B. 安全加固

| 任务 | 说明 | 状态 |
|---|---|---|
| 轻量安全过滤 | 关键词过滤 + 格式拦截（位于意图分类前） | ⏳ 未做 |
| 死执规则强化 | 扩展危险模式规则库、增加误报/漏报反馈机制 | ⏳ 未做 |

#### 6C. WebUI 面板

| 面板 | 来源 | 说明 | 状态 |
|---|---|---|---|
| 聊天面板 | **参考重写** fairy/channels/webui/static_html.py | 理解前端逻辑后重写为派蒙风格 | ✅ 早期实装 |
| 观测面板 | **参考重写** fairy/channels/webui/palace_html.py | 理解面板逻辑后重写，对接三月数据源 | ✅ 早期实装 |
| 知识/偏好面板 | **新建** | 草神提供数据 | ⏳ 未做（授权查看迁移到冰神·插件面板） |
| 理财面板 | **新建** | 岩神提供数据 | ⏳ 未做 |
| 游戏面板 | **新建** | 水神提供数据（后续） | ⏳ 未做 |
| 插件面板 | **新建** | 冰神提供数据（含授权管理 tab） | ✅ 2026-04-23 |
| 信息流面板 | **新建** | 风神提供数据 | ⏳ 未做 |
| **📨 推送收件箱** | **新建（本期额外）** | WebUI 固定会话收所有推送；QQ 不推送、Telegram 正常推送 | ✅ 2026-04-23 |

**Phase 6 验收**：
- WebUI 能查看/撤销永久授权；各面板有数据展示 —— 插件面板 ✅；其他业务面板未做
- 安全过滤能拦截危险输入 —— 未做
- **权限体系 + 安全过滤核心逻辑有单元测试覆盖** —— 未做
