/* Paimon — Lucide 图标内嵌 SVG 集合（精选 paimon 实际用到的）
 * MIT License (c) Lucide Contributors. https://lucide.dev
 *
 * 用法：const html = pmIcon('check') 或者 <span data-icon="check"></span> 自动替换
 */
(function () {
  const ICONS = {
    // nav
    'message-square': '<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>',
    'bar-chart-3': '<path d="M3 3v18h18"/><path d="M7 16V8"/><path d="M12 16V4"/><path d="M17 16v-6"/>',
    ...
    'puzzle': '<path d="M19.439 7.85c-.049.322.059.648.289.878l1.568 1.568c.47.47.47 1.229 0 1.698l-2.06 2.06a4 4 0 0 0-2.158-2.158l-2.06 2.06c.469.47.469 1.229 0 1.698l-1.569 1.568a1 1 0 0 1-.878.288c-.844-.128-1.741-.103-2.51.176-.766.276-1.448.812-1.992 1.357l-2.06-2.06c.47-.469 1.23-.469 1.699 0l1.568 1.569c.23.229.557.337.879.288.846-.13 1.74-.105 2.508.174.768.279 1.45.815 1.994 1.36"/>',
  ...
  /** 生成图标 SVG 字符串 — `pmIcon('check')` */
  window.pmIcon = function (name) {
    const inner = ICONS[name];
    if (!inner) {
      console.warn('[pmIcon] 未知图标:', name);
      return svgWrap(ICONS['circle']);
    }
    return svgWrap(inner);
  };

  /** 自动替换 [data-icon="xxx"] 元素内为对应 SVG */
  window.pmIcon.enhanceAll = function (root = document) {
    root.querySelectorAll('[data-icon]').forEach((el) => {
      const name = el.getAttribute('data-icon');
      if (!name || el.dataset.iconResolved === '1') return;
      el.innerHTML = window.pmIcon(name);
      el.dataset.iconResolved = '1';
    });
  };

  document.addEventListener('DOMContentLoaded', () => window.pmIcon.enhanceAll());
})();
