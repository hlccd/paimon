...
|------|------|----------------|---------|
| **自检-代码** | 对一份代码进行质量审查 | code | target |
| **自检-文档** | 对一份文档进行质量审查 | docs | target |
| **自检-方案** | 对技术方案/PRD 进行可实施性与完整性审查 | spec | target |
| **自检-PR** | 对一个 PR/MR 的变更代码进行质量审查 | pr | target（自动通过 git diff 限定范围） |

### 对比校验（双源审查）

| 类型 | 含义 | 内部 input_mode | 需要路径 | 特点 |
|------|------|----------------|---------|------|
| **代码 vs 文档**（默认） | 文档是否准确描述代码 | code-vs-docs | target + --docs | 以代码为准审文档 |
| **代码 vs 代码** | 两份代码各自打分 + 找差异 | code-vs-code | target + --src-b | 分别审查 + diff 视角 |
| **文档 vs 文档** | 两份文档各自打分 + 找差异 | docs-vs-docs | target + --docs-b | 分别审查 + diff 视角 |
| **代码 vs 方案** | 代码实现是否遵循技术方案 | code-vs-spec | target + --spec | 以方案为准审代码 |
| **变更 vs 方案** | 特定变更是否按方案执行 | change-vs-spec | target + --change + --spec | 以方案为准审变更 |
...
- 扫描 `src_path` 下所有代码文件，按语言统计，按目录分组（10-30 文件/组）
- 固定 report-only
- link-checker.py 不运行
...
- 扫描 `docs_path` 下所有 .md 文件（5-8 文件/组）
...
- 扫描 `spec_path` 下所有方案文件（.md/.docx/.pdf）
...
- 同时扫描 `src_path` 和 `docs_path`
- 文档侧可 `--fix`（仅改文档）
- link-checker.py 对 `docs_path` 运行

### 代码 vs 代码

- 分别扫描 target（A 侧）和 `--src-b`
...
- 同时扫描 `src_path`（代码）和 `spec_path`（方案）
...
- 扫描 `change_source`（变更范围）和 `spec_path`（方案）
- 变更来源支持三种输入：
