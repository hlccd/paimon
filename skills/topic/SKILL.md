...
stdout 「各源采集情况」段会列**全部 5 个源**（每个源一个 `### 源名` 子段）；最终 reply 的「各源讨论重点」必须**对应列出每一个源**，按状态写：

- 有数据：1-2 句基于本源 top 5 实际内容的事实归纳
- `⚠️ 缺 cookies`：原样写 `⚠️ 缺 cookies，去 webui /feed「站点登录」tab 扫码`
- `本次未拉到内容`：原样写 `本次未拉到内容`（**不要**自己揣测原因）
- `⚠️ 采集失败：xxx`：原样写 `⚠️ 采集失败：<错误信息>`

**不要省略缺数据的源**——用户需要知道为什么 Top 偏向某一两个源。
**不要替用户揣测原因**（比如把"未拉到"写成"cookies 失效"或"近期无讨论"）——用户自己能判断。
